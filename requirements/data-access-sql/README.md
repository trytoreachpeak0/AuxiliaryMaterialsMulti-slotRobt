...
```text
flow-sql-map/flows/*.yaml ──(sql_ids)──> sql-catalog/items/*.yaml ──(datasource)──> 逻辑数据源 ID
                                              │
                                              └──(tables)──> schema/tables/*.yaml
```
...
| `file` | string | 是 | 流程图文件路径，按仓库根目录相对路径记录。 | `requirements/flows/diagrams/操作员存入归还焊丝取出可用焊丝.md` |
