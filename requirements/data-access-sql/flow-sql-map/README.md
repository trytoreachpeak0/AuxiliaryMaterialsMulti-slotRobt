...
索引见 [index.yaml](index.yaml)。`sql_ids` 已与 `../sql-catalog/items/` 中的 `id` 对齐。
...
SQL/function 的输出字段以 `../sql-catalog/items/` 中的 `outputs` 为准；后续节点如果要使用上游输出，在自己的 `input_mappings.source_node/source_field` 中引用即可，避免双向重复维护。
...
- `access_type: mes_sql`：引用 `../sql-catalog/items/` 中的 MES 查询 SQL。
- `access_type: mes_function`：引用 `../sql-catalog/items/` 中的 MES PL/SQL/function 调用。
