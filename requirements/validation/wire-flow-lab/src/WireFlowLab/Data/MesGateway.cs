...
public sealed class OracleMesGateway : IMesGateway
{
    ...
    public DbResult Run(SqlCatalogItem item, IDictionary<string, object?> parameters)
    {
        ...
        var sql = useOutputParam ? rawSql : rawSql.TrimEnd().TrimEnd(';');
        var result = new DbResult { RenderedSql = sql };

        if (string.IsNullOrWhiteSpace(_connectionString))
            return DbResult.Failure(sql, "未配置 Oracle 连接串，无法连接真实 MES。请切换到 SQLite mock 或填写连接串。");
        ...
        try
        {
            ...
            if (useOutputParam)
            {
                var outParam = item.Id.Contains("wire_quota", StringComparison.OrdinalIgnoreCase)
                ...
                if (item.Id.Contains("wire_quota", StringComparison.OrdinalIgnoreCase))
                {
                    result.Rows.Add(new(StringComparer.OrdinalIgnoreCase) { ["quota_diff"] = outParam.Value?.ToString() });
                    result.RowsAffected = 1;
                }
                else
                {
